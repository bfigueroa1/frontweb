...
async function fetch_add_airplanes (id_ter, cantidad)	{ 
    ...
    let query = {
        "nivel": cantidad};
    let ruta = `http://localhost:3000/airplanes/${id_ter}`;
    let options = {
        method: 'PATCH',
        headers: {
...
async function fetch_add_ships (id_ter, cantidad)	{ 
    ...
    let query = {
        "nivel": cantidad};
    let ruta = `http://localhost:3000/ships/${id_ter}`;
    let options = {
        method: 'PATCH',
        headers: {
            'Accept': 'application/json',
...
async function fetch_add_wall(id_ter, cantidad)	{ 
    ...
    let query = {
        "nivel": cantidad};
    let ruta = `http://localhost:3000/walls/${id_ter}`;
    let options = {
        method: 'PATCH',
        headers: {
            'Accept': 'application/json',
...
async function fetch_ter_id(name) { 
    const rawResponse = await fetch(`http://localhost:3000/territories/get_id/${name}`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
