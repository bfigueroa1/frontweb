...
async function fetch_getAllPlayers(id_game) {  //CONSEGUIR TODO LOS PLAYERS ID DEL GAME
    const rawResponse = await fetch(`https://infinite-escarpment-66062.herokuapp.com/games/username/${id_game}`, {
        method: 'GET',
        headers: {
...
async function fetch_army_id(id_player) {  //CONSEGUIR TODO LOS PLAYERS ID DEL GAME
    const rawResponse = await fetch(`https://infinite-escarpment-66062.herokuapp.com/armies/get_army_id/${id_player}`, {
        method: 'GET',
        headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
